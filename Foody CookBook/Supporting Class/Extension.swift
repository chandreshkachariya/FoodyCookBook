...
public extension UIViewController {
    func showToast(message : String, delay: Float) {
        ...
        toastLabel.font = getFont(fontName: _font_Montserrat_Regular, fontSize: 15.0)// UIFont(name: "Montserrat-Light", size: 12.0)
...
extension TimeInterval {
    var time: String {
        //return String(format:"%02d:%02d", Int(self/30),  Int(ceil(truncatingRemainder(dividingBy: 30))) )
        print(millisecond)
        if millisecond == 0 {
            return String(format:"%2d.%ds", 29-second, millisecond)
        }
        return String(format:"%2d.%ds", 29-second, 10-millisecond)
    }
    ...
    var millisecond: Int {
        Int((self*1000).truncatingRemainder(dividingBy: 1000)/100)
    }
}
extension Int {
    ...
    //Format 1000's into a friendly 1 K's
