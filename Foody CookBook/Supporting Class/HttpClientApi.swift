...
class HttpClientApi: NSObject {
    ...
    func callAPIWithImagesAndVideo(url: String,
                            params: [String: Any]?,
                            showLoading : Bool,
                            arrImage:[String: UIImage]?,
                            arrVideo:[String: URL]?,
        ...
        AF.upload(multipartFormData: { multipartFormData in
            ...
            if arrVideo != nil {
                for (key, value) in arrVideo! {
                    let timestamp = NSDate().timeIntervalSince1970
                    multipartFormData.append(value, withName: key, fileName: "\(timestamp).mp4", mimeType: "\(timestamp)/mp4")
                }
            }
   ...
   func callAPIWithBodyData(url: String,
    ...
    dictParams.setValue("I", forKey: "device_type")
    ...
    func callAPIForGetMusic(url: String,
                 params: Dictionary<String, Any>?,
                 showLoading : Bool,
                 methods: HttpMethod,
                 success:@escaping (_ _result : Any? ) -> Void,
        ...
        let headers: HTTPHeaders = [
            "x-rapidapi-key": "292482a590msheb5d904a39725d2p1a57ecjsn0b5a0a5317d2",
            "x-rapidapi-host": "deezerdevs-deezer.p.rapidapi.com",
        ]
        
        AF.request(url, method: HTTPMethod(rawValue: methods.rawValue) , parameters: param, headers: headers)
             .responseJSON(completionHandler: { response in
